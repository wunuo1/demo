    ...
    : rclcpp::Node(node_name, options)
{
    ...
    image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
                "/nebula200/mtof_rgb/image_raw", 10, std::bind(&NavLocNode::sub_cam_callback, this, std::placeholders::_1));
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("/home/wunuo/final-voxel.pcd", pcl_cloud_) == -1)
    {
...
void NavLocNode::init_param(){
    ...
    tf2::Vector3 t_imu_tof(-0.0039799203111821468, -0.14001955020902342461, -0.027189542185995823831);
    ...
    if(build_map_ == false) {
        YAML::Node config = YAML::LoadFile("/home/wunuo/demo_ws/src/demo/output/config.yaml");
...
void NavLocNode::mean_transform(const std::vector<tf2::Transform> &tf_tmp)
{
    ...
    if(build_map_ == true){
        ...
        std::ofstream fout("/home/wunuo/demo_ws/src/demo/output/config.yaml");
        fout << node;
        fout.close();
...
void NavLocNode::client_control(){
    ...
    while (process_stop_ == false){
        ...
        if(process_stop_ == false){
            ...
            goal_msg.pose.pose.position.x = 1.29;
            goal_msg.pose.pose.position.y = -3.28;
            goal_msg.pose.pose.orientation.z = -0.7512804;
            goal_msg.pose.pose.orientation.w = 0.6599831;
            ...
            
            {
                ...
                goal_msg.pose.header.frame_id = "map";
                goal_msg.pose.header.stamp = this->get_clock()->now();
                goal_msg.pose.pose.position.x = -0.41;
            ...
            }
            {
                ...
                task_cv_.wait(lock, [&]{ return task_done_; });
                task_done_ = false;
                std::this_thread::sleep_for(std::chrono::seconds(2)); // 模拟任务处理
                has_pub_ = false;
                mean_tf_v_.clear();
            }
